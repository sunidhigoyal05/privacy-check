  ...
  return (
    <div className="overflow-x-auto">
      <table className="w-full">
        <thead>
          <tr>
            <th className="text-left text-xs font-medium text-[#6666A0] pb-3 pr-4">Data Type</th>
            {STAGES.map(stage => (
              <th key={stage} className="text-center text-xs font-medium text-[#6666A0] pb-3 capitalize px-2">
                {stage}
              </th>
            ))}
          </tr>
        ...
        <tbody>
          {data.map((row, i) => (
            <tr key={i}>
              <td className="text-sm text-[#AAAACE] py-1.5 pr-4 capitalize font-medium">{row.data_type ?? '—'}</td>
              {STAGES.map(stage => {
                const value = Number(row[stage]) || 0;
                return (
                  <td key={stage} className="px-1 py-1.5">
                    <Tooltip content={`${row.data_type ?? stage} → ${stage}: ${Math.round(value * 100)}% risk`}>
